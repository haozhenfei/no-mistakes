...
| 12 | verify 配置 | `.no-mistakes.yaml` 增 `verify: {skeptics: 3, agents: [...], replay_sample: 0.2, auto_fix.verify: N}` | 配置点；**实现里 `skeptics` 默认值是 1**（成本取舍），即默认没有多数表决，单个 skeptic 说了算，要多数表决须显式配 >= 3 |
