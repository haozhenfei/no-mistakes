...
- `agent_path_override`, `agent_args_override`, `acpx_path`, `acp_registry_overrides`, `ci_timeout`, `daemon_connect_timeout`, `run_setup_timeout`, `step_quiet_warning`, `log_level`, and `session_reuse` are global-only fields.
- `commands`, `ignore_patterns`, `document.instructions`, `review.instructions`, and `allow_repo_commands` are repo-only fields. By default the gate-control fields — `commands`, `agent`, `test.evidence.upload_cmd`, `document.instructions`, and `review.instructions` — are read from the trusted default branch, not the branch being gated. An `allow_repo_commands: true` opt-in reads all of them from the branch being gated instead; the opt-in itself is only settable by the maintainer (trusted default branch, or `repos.<path>` in global config), never by a pushed branch. See the [Repo Config Reference](/no-mistakes/reference/repo-config/) security note.
