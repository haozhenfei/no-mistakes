...
- **Fixed pipeline** - the step order is opinionated and not configurable: `intent → rebase → fix → review → test → document → lint → push → pr`. It ends at the PR, and a [watch run](/no-mistakes/concepts/pipeline/#watch-runs) monitors the PR from there. (An adversarial `verify` step exists in code but is disabled by default; see [Pipeline](/no-mistakes/concepts/pipeline/).) What you _can_ configure is the commands each step runs, how many auto-fix attempts are allowed, and whether transcript-based intent extraction is used when intent is not supplied directly.
