...
- A fixed, opinionated pipeline: `intent → rebase → fix → review → test → verify → document → lint → push → pr`. Order is not configurable; what each step runs is. It ends at the PR; a [watch run](/no-mistakes/concepts/pipeline/#watch-runs) monitors the PR from there.
