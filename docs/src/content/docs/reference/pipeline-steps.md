...
- Classifies every changed hunk against the instrumentation into one of `executed`, `not-executed`, `uninstrumented`, or `no-data`, and records it on the ledger row beside the evidence state. `not-executed` means the engine watched those lines (or the statement enclosing them) and recorded zero hits. `uninstrumented` means the engine emitted no record for them at all while the statement enclosing them did execute — a coverage engine that structurally cannot instrument a construct (v8 emits no line record for a JSX attribute line, folding it into the enclosing `return`) is not evidence that the code never ran
- Requires a behavior-class claim (`behavior`, `regression-fixed`) to be backed by runtime evidence: at least one executed hunk, resolved against the hunks the claim names with `claim add --hunks`, or run-wide when it names none. A claim whose hunks are `not-executed`, or for which nothing was captured at all, is capped at `PLAUSIBLE` — never `CONFIRMED` — whatever the skeptic said, and the gate parks. A coverage audit that could not run backs nothing either
- A claim backed only by `uninstrumented` hunks is NOT capped: the skeptic's verdict stands and the run does not park, because the machine did not observe the code failing to run — it could not observe those lines at all. The blindness is reported as a `COVERAGE BLIND SPOT` warning and the hunks stay out of the runtime-verified count and visible in the dossier. A single `not-executed` hunk among a claim's hunks still sinks it, blind siblings notwithstanding
- Surfaces the remaining coverage audit issues as non-parking findings

**Approval:** required when verify REFUTES a claim, and when a behavior-class claim has no runtime evidence behind it. Coverage blind spots, hunks instrumentation saw never execute, and other coverage-audit warnings are recorded for routing and dossier visibility but do not park the run by themselves.
