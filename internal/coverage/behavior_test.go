...
// exactly those hunks.
func TestUnbackedBehaviorClaims_AttestedHunksBackNothing(t *testing.T) {
	ledger := []LedgerEntry{
	...
	gaps := UnbackedBehaviorClaims([]claims.Claim{
...
func TestUnbackedBehaviorClaims_RuntimeVerifiedHunkBacksTheClaim(t *testing.T) {
	ledger := []LedgerEntry{
...
func TestUnbackedBehaviorClaims_RunWideFallback(t *testing.T) {
	noRuntime := []LedgerEntry{{File: "row.tsx", StartLine: 1, EndLine: 2, State: StateAttested}}
	if gaps := UnbackedBehaviorClaims([]claims.Claim{behaviorClaim("c1", claims.KindBehavior)}, noRuntime); len(gaps) != 1 {
	...
	if gaps := UnbackedBehaviorClaims([]claims.Claim{behaviorClaim("c1", claims.KindBehavior)}, someRuntime); len(gaps) != 0 {
...
func TestUnbackedBehaviorClaims_EmptyLedgerBacksNothing(t *testing.T) {
	if gaps := UnbackedBehaviorClaims([]claims.Claim{behaviorClaim("c1", claims.KindBehavior)}, nil); len(gaps) != 1 {
...
func TestUnbackedBehaviorClaims_NonBehaviorKindsAndSelfAttestedAreExempt(t *testing.T) {
	cs := []claims.Claim{
	...
	if gaps := UnbackedBehaviorClaims(cs, nil); len(gaps) != 0 {
