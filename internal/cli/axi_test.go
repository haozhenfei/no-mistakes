...
func TestRerunParamsIncludeSkipSteps(t *testing.T) {
	params := rerunParams("repo-1", "feature/x", stepSelection{skip: []types.StepName{types.StepReview}}, false, "user goal")
...
func TestAxiRunReportsInvalidGlobalConfig(t *testing.T) {
	...
	cmd.SetContext(context.Background())
	cmd.SetOut(&out)
	if err := runAxiRun(cmd, false, stepSelection{}, false, "user goal"); err == nil {
