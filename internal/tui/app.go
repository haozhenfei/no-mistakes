...
func NewModel(socketPath string, client *ipc.Client, run *ipc.RunInfo) Model {
	...
	m := Model{
		...
		subscriptionID:      1,
		run:                 run,
		done:                run.Status == types.RunCompleted || run.Status == types.RunFailed || run.Status == types.RunCancelled || run.Status == types.RunInterrupted,
		steps:               steps,
...
func (m Model) terminalTitle() string {
	...
	if m.done || m.run == nil {
		...
		case m.run.Status == types.RunInterrupted:
